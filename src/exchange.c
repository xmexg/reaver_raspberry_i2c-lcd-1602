              ...
              !premature_timeout)
	{
		...
		switch(packet_type)
		{
			case IDENTITY_REQUEST:
				cprintf(VERBOSE, "[+] 收到的身份请求\n");
				tx_type = IDENTITY_RESPONSE;
				id_response_sent = 1;
				break;
			case M1:
				cprintf(VERBOSE, "[+] 收到 M1 信息\n");
			...
			case TERMINATE:
				...
				break;
			default:
				cprintf(VERBOSE, "[!] 警告: 收到意外的数据包 (0x%.02X), 正在停止破解\n", packet_type);
				terminated = 1;
				break;
		}
